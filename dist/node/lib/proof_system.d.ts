import { Bool, AsFieldElements, Pickles } from '../snarky.js';
import { Context } from './global-context.js';
export { Proof, SelfProof, ZkProgram, verify };
export { CompiledTag, sortMethodArguments, getPreviousProofsForProver, MethodInterface, GenericArgument, picklesRuleFromFunction, compileProgram, analyzeMethod, emptyValue, emptyWitness, synthesizeMethodArguments, methodArgumentsToConstant, methodArgumentTypesAndValues, isAsFields, snarkContext, inProver, inCompile, inAnalyze, inCheckedComputation, };
...
declare namespace ZkProgram {
    var Proof: <PublicInputType extends AsFieldElements<any>>(program: {
        name: string;
        publicInputType: PublicInputType;
    }) => {
...
declare function isAsFields(type: unknown): type is AsFieldElements<unknown> & ObjectConstructor;
...
declare function analyzeMethod<T>(publicInputType: AsFieldElements<any>, methodIntf: MethodInterface, method: (...args: any) => T): {
    rows: number;
    digest: string;
    result: T;
};
...
declare function emptyValue<T>(type: AsFieldElements<T>): T;
declare function emptyWitness<T>(type: AsFieldElements<T>): T;
